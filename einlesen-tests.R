...
radbet <- subset(vums2016, X01 == 71 | X02 == 71)
dim(radbet) # 953 Unfälle mit Radbeteiligung

# der eine Unfall mit einem Radfahrer-Toten:
subset(radbet, T == 1)

# Wer sind Hauptverursacher bei Unfällen mit Radfahrerbeteiligung?
table(radbet$X01)
table(radbet$X02, useNA = "always") # 77 NAs -> Alleinunfälle

radbetnall <- subset(radbet, !is.na(X02))
dim(radbetnall) # 876 Unfälle mit Radfahrerbeteiligung ohne Alleinunfälle
